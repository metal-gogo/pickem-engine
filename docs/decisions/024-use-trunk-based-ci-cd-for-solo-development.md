...
The project is currently developed by one person, but it still needs a CI/CD process that keeps `main` trustworthy, supports future GitHub Actions checks, and leaves room for staging, production, and pull-request database environments.
...
- pull requests may use preview deployments and short-lived Neon branches
- staging should be a Cloudflare/Neon environment deployable from `main` or a selected commit
...
This model works well with the earlier Neon environment decision because database branches and deployment environments can represent `dev`, `staging`, `prod`, and pull-request previews without requiring matching long-lived git branches.
...
- Use GitHub Environments for staging and production secrets and approval gates where useful.
- Add pull-request preview deployments and Neon preview branches later when the backend setup justifies them.
...
- Configure GitHub Environments for staging and production when deployment workflows are introduced.
