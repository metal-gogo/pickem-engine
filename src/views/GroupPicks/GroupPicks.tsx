...
import { pageStackClass, panelHeaderClass, sectionCopyClass, sectionPanelClass, sectionTitleClass, surfaceClass, tileCardClass } from "../../app/ui";
  ...
  return (
    <div className={pageStackClass}>
      <section className={sectionPanelClass}>
        <div className={panelHeaderClass}>
          <div className="grid gap-3">
            <Link className="inline-flex w-fit items-center gap-2 text-sm font-bold text-app-muted transition-colors hover:text-app-ink" to={`/pools/${pool.id}`}>
        ...
        <div className="grid gap-3.5 lg:grid-cols-2">
          <div className={tileCardClass}>
            ...
            <span className="font-display text-lg tracking-[-0.02em] text-app-ink">{formatSavedAt(savedPickSet.updatedAt)}</span>
            <span className="leading-7 text-app-muted">{completedSavedCount} saved picks currently shape this group table.</span>
          </div>
          <div className={tileCardClass}>
            <span className="font-bold text-app-ink">Navigation</span>
            <span className="font-display text-lg tracking-[-0.02em] text-app-ink">
      ...
      <section className={cn(surfaceClass, "flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between lg:p-6")}>
        <div>
          <h3 className="mb-1.5 font-display text-[1.35rem] tracking-[-0.02em] text-app-ink">
